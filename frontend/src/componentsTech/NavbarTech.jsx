      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <Link to="/" className="text-2xl font-bold text-indigo-600 tracking-wide">
            EventTracker <span className="text-sm font-medium text-gray-600">(Tech)</span>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-6">
            {user ? (
              <>
                <Link to="/technology" className={navLinkClasses}>
                  Dashboard
                </Link>
                <Link to="/technology/events" className={navLinkClasses}>
                  Events
                </Link>
                <Link to="/external-events" className={navLinkClasses} state={{ from: '/technology' }}>
                  External Events
                </Link>
                {(user.role === "masterAdmin" || user.role === "superAdminTech" || user.role.endsWith("TechHod")) && (
                  <Link to="/technology/create-event" className={navLinkClasses}>
                    Create Event
                  </Link>
                )}
                <motion.button
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {user ? (
                <>
                  <Link to="/technology" className={navLinkClasses} onClick={toggleMobileMenu}>
                    Dashboard
                  </Link>
                  <Link to="/technology/events" className={navLinkClasses} onClick={toggleMobileMenu}>
                    Events
                  </Link>
                  <Link to="/external-events" className={navLinkClasses} state={{ from: '/technology' }} onClick={toggleMobileMenu}>
                    External Events
                  </Link>
                  {(user.role === "masterAdmin" || user.role === "superAdminTech" || user.role.endsWith("TechHod")) && (
                    <Link to="/technology/create-event" className={navLinkClasses} onClick={toggleMobileMenu}>
                      Create Event
                    </Link>
                  )}
                  <motion.button
